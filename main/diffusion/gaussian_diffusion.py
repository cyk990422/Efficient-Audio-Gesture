...
class GaussianDiffusion:
    ...
    #     mask_1= torch.ones_like(mask).cuda().float()
    #     loss = self.smooth_l1_loss(a, b)
    #     loss = sum_flat(loss * mask_1.float())  # gives \sigma_euclidean over unmasked elements
    #     n_entries = a.shape[1] * a.shape[2]
    #     non_zero_elements = sum_flat(mask_1) * n_entries

        
    #     non_zero_elements = torch.where(non_zero_elements == 0.0, torch.tensor(1.0).cuda(), non_zero_elements).cuda()
    ...
    def p_mean_variance(
        ...
        B, C = x.shape[:2]
        assert t.shape == (B,)
        latent_z_upper = torch.randn(B, 64, device='cuda')
        latent_z_lower = torch.randn(B, 64, device='cuda')
        latent_z_exp = torch.randn(B, 64, device='cuda')
        latent_z_hands = torch.randn(B, 64, device='cuda')

        #model_output,output_upper,output_hands,output_lower,output_exp = model(x, self._scale_timesteps(t), **model_kwargs)
        model_output,output_upper,output_hands,output_lower,output_exp = model(x, self._scale_timesteps(t), latent_z_upper,latent_z_lower,latent_z_hands,latent_z_exp,**model_kwargs)
    ...
    def sample_posterior(self, x_0,x_t, t):
        def q_posterior(x_0, x_t, t):
            ...
            log_var_clipped = _extract_into_tensor(self.posterior_log_variance_clipped, t, x_t.shape)
...
def _extract_into_tensor_no_expand(arr, timesteps, broadcast_shape):
    """
    Extract values from a 1-D numpy array for a batch of indices.
