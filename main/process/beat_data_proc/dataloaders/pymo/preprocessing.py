    ...
    def transform(self, X, y=None):
        if self.param_type == "euler":
            ...
            raise UnsupportedParamError(
                "Unsupported param: %s. Valid param types are: euler, quat, expmap, position" % self.param_type
            )
    ...
    def _to_expmap(self, X):
        ...
        for track in X:
            ...
            for joint in joints:
                ...
                euler = [
                    [f[1]["%s_Xrotation" % joint], f[1]["%s_Yrotation" % joint], f[1]["%s_Zrotation" % joint]]
                    for f in r.iterrows()
                ]  # Make sure the columsn are organized in xyz order
                exps = [
                    Rotation(f, "euler", from_deg=True).to_expmap() for f in euler
